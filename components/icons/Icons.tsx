import React from 'react';
import { Plus, X, Search, Clock, Globe, Monitor, RefreshCw, MapPin, CircleHelp } from 'lucide-react';

export const PlusIcon = ({ className }: { className?: string }) => <Plus className={className} />;
export const XIcon = ({ className }: { className?: string }) => <X className={className} />;
export const SearchIcon = ({ className }: { className?: string }) => <Search className={className} />;
export const ClockIcon = ({ className }: { className?: string }) => <Clock className={className} />;
export const GlobeIcon = ({ className }: { className?: string }) => <Globe className={className} />;
export const MonitorIcon = ({ className }: { className?: string }) => <Monitor className={className} />;
export const ResetIcon = ({ className }: { className?: string }) => <RefreshCw className={className} />;
export const MapPinIcon = ({ className }: { className?: string }) => <MapPin className={className} />;
export const HelpIcon = ({ className }: { className?: string }) => <CircleHelp className={className} />;